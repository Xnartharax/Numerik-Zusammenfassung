...
\begin{document}
	...
	\begin{algorithm}
		...
		\begin{algorithmic}
			...
			\For{$i\in 1:n$}
				\For{$j \in i+1:n$}
					\State $a_{ji} \leftarrow \frac{a_{ji}}{a_{ii}}$
					\For{$k\in i+1:n$}
	...
	\begin{algorithm}[H]
		...
		\begin{algorithmic}
			...
			\For{$i\in 1:n$}
				...
				\State $P, A,\leftarrow P^{(p, i)} P, P^{(p, i)}A$
				\For{$j \in i+1:n$}
					\State $a_{ji} \leftarrow \frac{a_{ji}}{a_{ii}}$
					\For{$k\in i+1:n$}
					 	\State $a_{jk} \leftarrow a_{jk} - a_{ji}a_{ji}$
	...
	\begin{definition}[Lineares Ausgleichsproblem]
		Sei $A\in \mathbb{R}^{m\times n}, b\in \mathbb{R}^n$ mit $m\leq n$. Gesucht ist $x\in \mathbb{R}^m$. Sodass $$||Ax-b||_2$$ minimiert wird. Es wird im Generellen angenommen, dass $Rang(A) = m$.
	\end{definition}
	...
	\begin{algorithm}
		...
		\begin{algorithmic}
			\Require Ausgleichsproblem $A\in \mathbb{R}^{n\times m}, b\in \mathbb{R}^m, m\leq n$
	...
	\begin{theorem}[Konvergenz der Vektoriteration]
		Sei $A\in \mathbb{R}^{n \times n}$ symmetrisch mit EW $|\lambda_n| \leq ... \leq |\lambda_2| < |\lambda_1|$ und EV $(v_i)_{i\in [n]}$ und  $\beta_1 := v^T_1x^{(0)} \neq 0$. Dann gilt:
		...
		und $C_1, C_2$ Konstanten sind die vom Startwertabhängen. Ist $A$ nicht symmetrisch aber noch normal gilt nur noch Konvergenz:
		$$|\lambda^{(i)}-\lambda_1| \leq C_1(x^{(0)})\vert\frac{\lambda_2}{\lambda_1}\vert^{i}$$
	...
	\begin{theorem}[Bauer-Fike, Kondition des Eigenwertproblems für diagonalisierbare Matrizen]
		...
		$$|\mu-\lambda_\mu|\leq \Vert T\Vert_p\Vert T^{-1}\Vert_p\Vert\Delta A\Vert_p$$
	...
	\begin{definition}
		...
		$$s(x)|_{[x_{i-1}, x_i]} = c_i + d_i(x-\frac{x_{i-1}+x_i}{2}) +m_i\frac{(x-x_{i})^3}{6h_i} - m_{i-1}\frac{(x-x_{i-1})^3}{6h_i}$$
